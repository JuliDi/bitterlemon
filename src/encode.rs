...
impl Run {
    fn size(&self) -> u8 {
        // TODO rename this to len for Rusticity
        match *self {
            Run::Set(x) => x,
            Run::Clear(x) => x,
        }
...
mod test_run_builder {
    ...
    fn runs() {
        let op = |input: &[u8], output: &[Run]| {
            ...
            let collected: Vec<Run> = RunIterator::from_bits(bool_stream.iter().copied()).collect();
    ...
    fn next_none_idempotence() {
        let mut iter = RunIterator::from_bits((&[] as &[bool]).iter().copied());
        for i in 0..20 {
            let next = iter.next();
            assert_eq!(
                None, next,
                "pulling next() on run {} was {:?}, not None",
                i, next
            );
        }
    }
...
impl RunHoldingExtensions for RunHolding {
    ...
    fn num_bits(&self) -> u8 {
        ...
        debug_assert!(
            r <= RLE_MAX_FRAME as usize,
            "number of frame bits too high at {:?}",
            r
        );
        r as u8
    }
        ...
        let (to_return, next_mode): (Option<u8>, Option<WithFramesMode>) = match self.mode {
            WithFramesMode::Filling => {
                if self.runs.len() == 1 {
                    ...
                    (
                        Some(if header_byte == RLE_MAX_FRAME {
                            0u8
                        } else {
                            header_byte
                        }),
...
mod test_with_frames {
    ...
    #[test]
    fn next_none_idempotence() {
        let mut iter = WithFrames::new((&[] as &[Run]).iter().copied());
        for i in 0..20 {
            let next = iter.next();
            assert_eq!(
                None, next,
    ...
    fn avoid_frame_overflow() {
        ...
        outputs.extend_from_slice(&std::vec![0xaa; RLE_MAX_FRAME as usize / 8]);
        outputs.push(0xc1);
